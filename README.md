...
| 10 | `supabase/migrations/0010_pnl_snapshots.sql` | P&L KPI 스냅샷(추이 트래킹) |
| 11 | `supabase/migrations/0011_product_shots.sql` | 제품컷 라이브러리 |
...
### P&L (`/pnl`)
구글 P&L 시트를 읽어 핵심 KPI(총매출·순매출·영업이익·이익률·광고비·ROAS·환불률·주문건수·객단가)를
표시하고, "지금 스냅샷 저장" 또는 평일 아침 자동으로 `pnl_snapshots` 에 기록해 추이를 트래킹한다.
시트/gid 는 `app_settings` 의 `pnl_sheet_id`·`pnl_gid` 로 바꾼다. 시트는 링크·뷰어 공유 필요. 마이그레이션 `0010`.

### 통합 P&L (`/pnl/integrated`)
운호컴퍼니(주당의비결·뷰티밤·파트너사) + 대운목장 + 신미집 5개 브랜드의 손익을 한 화면에서 본다.
구글 시트 `UC_운영도구_P&L`의 **매출원장·매입원장·계정매핑** 세 탭을 그대로 가져와
`src/data/pnl-ledger.json` 에 적재하고(매출 2,210행 / 매입 2,139행), 시트의 파생 탭(월간·주간·연간
P&L, 대시보드)은 가져오지 않고 원장에서 같은 규칙으로 다시 계산한다. 그래서 기간·브랜드·채널을
어떻게 잘라도 같은 손익이 나온다.
...
| 라인 | 산식 |
|---|---|
| 매출 | 매출원장 **순매출** 중 손익반영 행만. 정산입금·차입금·투자금·매입 오기재는 제외 |
| 매출원가 | 매입원장 구분=매출원가 (상품매입·식자재·주류·원부자재·포장재·물류비) |
| 판관비 | 매입원장 구분=판관비 + 매출원장 결제·판매수수료 |
| 영업외 | 매입원장 구분=영업외비용 (이자비용) |
| 제외 | 재무거래·내부거래·카드대금·환불·미분류 → 손익에서 빼고 검증 블록에 표시 |

화면 구성: 브랜드 탭(전사/5개 브랜드/미지정) · 월간·주간(분기 13주)·연간(5개년) · 기간별
손익계산서 · 브랜드별 손익 · 채널별 매출 · 계정과목별 매입 · 매입원장 대사 검증 블록.
`/pnl/integrated/ledger` 에서 원장 원본을 연·월·브랜드·채널/계정과목·검색어로 조회한다.

**계산 기준 토글** — `시트 그대로`는 매입원장 자동열(계정과목·구분)을 그대로 써서 시트 P&L 탭과
값이 정확히 일치한다. 시트에서 자동열이 비어 있는 21건은 시트와 마찬가지로 손익에 잡히지 않는다.
`매핑표 보정`은 그 21건을 계정매핑표로 채워 다시 계산한다.

`tests/integrated-pnl.test.ts` 가 시트의 월간·주간·연간 P&L 탭 전 라인과 대조한다
(`npm test`). 기대값 `tests/pnl-sheet-expected.json` 은 시트에서 그대로 뽑은 값이다.

### 제품컷 라이브러리 (`/library`)
브랜드별 실제 제품 사진을 올려두면 디자이너 에이전트가 배치 지시서에 활용한다(AI가 이미지를
생성하지 않고 실제 컷을 배치). 파일은 Storage 공개 버킷 `generated-media` 사용. 마이그레이션 `0011`.

### 승인 후 다음 액션
승인 큐에서 **승인하면 집행 업무가 자동 생성**되어 업무 보드에 "예정"으로 뜬다(에이전트별 카테고리).
담당자가 집행(촬영·업로드·발송) 후 리포트에서 성과를 기록하는 흐름으로 이어진다.
규제 검수 미통과분도 승인 큐에 노출되며(지적 문구·대체 제안 표시) 대표가 최종 판단한다.

### 설정 진단 (`/settings`, 대표)
환경변수·Storage 버킷·시트 읽기·카카오 연결 상태를 한눈에 점검하고, 사용자 목록을 확인한다.
...
- **성과 CSV 업로드**: 리포트 화면 → "성과 CSV 업로드". 채널(스마트스토어·메타·인스타·GA 등)에서
  내보낸 CSV를 올리면 `performance` 에 반영되고 대시보드 "최근 7일 성과"·마케터 피드백 루프에 쓰인다.
  헤더 예: `일자,브랜드,채널,도달,전환,매출` (한/영 인식). 브랜드는 브랜드명 또는 약어(rb·jd…)로 매칭.
- **셀러 구글시트(읽기 전용)**: 셀러 화면(`/sellers`)에서 시트를 라이브로 읽는다(로컬 복제 없음).
  MD 에이전트가 이 데이터를 후보 검증 입력으로 사용한다.
  시트가 읽히려면 구글 시트 공유를 **"링크가 있는 모든 사용자 · 뷰어"**(읽기 전용)로 설정한다.
  다른 시트를 쓰려면 `app_settings` 의 `seller_sheet_id` 를 바꾼다(기본값은 CLAUDE.md 시트 ID).
