...
-- ── 법인마스터 ──────────────────────────────────────────────────────────────
insert into public.corporations (name, kind, business_no, founded, ceo, address, confirmed, note) values
  ('㈜운호컴퍼니', 'own', '715-87-03094', '2024.02', '최운호',
   '서울 강남구 신사동 562-17 신영빌딩 301호', '확정',
   '지주 법인. 2019년은 개인사업자 개시 시점으로 법인 설립연월(2024.02)과 구분해 표기한다.'),
  ('뷰티밤㈜', 'own', '654-81-02686', '2022.02', '최운호',
   '서울 강남구 신사동 562-17 신영빌딩 301호', '확정',
   '㈜운호컴퍼니가 지분 전량 인수 예정. 인수 완료 전까지 별개 당사자로 취급.'),
  ('SBJ컴퍼니', 'partner', null, null, '박경배',
   '서울 강남구 청담동', '부분 확정',
   '청담오리골 사업자. 자사 법인 아님. 사업자번호·상세 주소 확인 후 제안서 당사자로 사용.'),
  ('엣지라인의원', 'affiliate', null, null, null, null, '미확정',
   '등기부상 최운호 명의 없음. 서면 계약 없이 마케팅 성과 기반 수익 쉐어. 약정서 체결 필요.')
on conflict (name) do update set
  kind = excluded.kind, business_no = excluded.business_no, founded = excluded.founded,
  ceo = excluded.ceo, address = excluded.address, confirmed = excluded.confirmed,
  note = excluded.note;
