# 하루바른·나아 운영 통합 플랫폼 — Phase 1

> **이 앱은 운호컴퍼니 운영 플랫폼과 동일한 구조를 하루바른·나아 두 브랜드용으로 복제한 별도 앱이다.**
> (같은 저장소 안 `harubareun/` 폴더. 운호 앱과 코드가 완전히 분리돼 독립 배포 가능.)
>
> **운호 대비 달라진 점**
> - 제외된 폴더: `CEO 투두`, `인적자산(인맥)`, `명함목록`, `안티에이징 관리` — 라우트·네비·관련 크론/브리핑 참조까지 제거.
> - 브랜드 마스터(`supabase/migrations/0003_seed_brand_master.sql`)를 **하루바른(hb)·나아(na)** 로 교체. 카테고리·규제·주력제품·VI·거래처는 **임시 기본값**이라 실제 정보 확정 후 교체한다.
> - 사이드바·로그인·매니페스트·이메일·AI 프롬프트 등 브랜드 표기를 하루바른·나아로 교체.
> - 매장 운영(신미집·대운목장·다이닝·F&B) 모듈은 "폴더·기능 동일" 요청에 따라 구조 그대로 유지(라벨만 중립화). 하루바른·나아에 매장이 없다면 사용하지 않으면 된다.
>
> 나머지 UI·폴더명·기능은 운호 플랫폼과 동일하다. 아래 설치·운영 안내도 그대로 적용된다.
