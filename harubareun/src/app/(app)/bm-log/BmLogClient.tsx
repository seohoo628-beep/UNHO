...
import { createBmLog, updateBmLog, deleteBmLog } from "./actions";
...
function Fields({ log, users, today }: { log?: BmLog; users: Opt[]; today: string }) {
  return (
    <>
      <div className="row">
...
function EditForm({ log, users, today, onDone }: { log: BmLog; users: Opt[]; today: string; onDone: () => void }) {
  const [err, setErr] = useState<string | null>(null);
  const [pending, start] = useTransition();
  const router = useRouter();
  ...
  function submit(fd: FormData) {
    setErr(null);
    start(async () => {
      const r = await updateBmLog(log.id, fd);
      if (!r.ok) {
        setErr(r.error ?? "저장 실패");
        return;
      }
...
function LogRow({ log, users, today }: { log: BmLog; users: Opt[]; today: string }) {
...
export default function BmLogClient({
  logs,
  users,
  today,
  dbReady,
