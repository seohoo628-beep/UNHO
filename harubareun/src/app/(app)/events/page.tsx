...
export default async function Page() {
  ...
  let q = supabase.from("marketing_events")
    .select("id, brand_id, status, title, rationale, benefit_type, channel, start_date, end_date, target_revenue, actual_revenue, buyers, ad_cost, note, brands(name)")
  ...
  const items = ((data ?? []) as any[]).map((r) => ({
    ...
    targetRevenue: r.target_revenue, actualRevenue: r.actual_revenue, buyers: r.buyers, adCost: r.ad_cost, note: r.note,
  })) as Ev[];
