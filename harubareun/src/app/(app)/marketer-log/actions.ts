...
const MARKETER_LOG_KINDS = ["일일업무일지", "주간업무계획", "월간업무계획"] as const;
...
function isMissingTable(err: { code?: string; message?: string } | null): boolean {
  if (!err) return false;
  return err.code === "42P01" || /marketer_logs/.test(err.message ?? "");
}
...
function normKind(v: FormDataEntryValue | null): string {
  ...
  return (MARKETER_LOG_KINDS as readonly string[]).includes(s) ? s : "일일업무일지";
}

export async function createMarketerLog(formData: FormData): Promise<Result> {
  let user;
  try {
  ...
  const { error } = await supabase.from("marketer_logs").insert(row);
  if (error) return { ok: false, error: error.message, tableMissing: isMissingTable(error) };
  await logAudit({ actorId: user.id, actorName: user.name, action: "created", entity: "marketer_log", label: `${kind} ${logDate}` });
  revalidatePath("/marketer-log");
  return { ok: true };
}

export async function updateMarketerLog(id: string, formData: FormData): Promise<Result> {
  try {
    await guardUser();
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : "권한 오류" };
  ...
  const { error } = await supabase.from("marketer_logs").update(patch).eq("id", id);
  if (error) return { ok: false, error: error.message };
  revalidatePath("/marketer-log");
  return { ok: true };
}

export async function deleteMarketerLog(id: string): Promise<Result> {
  let actor;
  try {
  ...
  const supabase = createSupabaseServerClient();
  const { error } = await supabase.from("marketer_logs").delete().eq("id", id);
  if (error) return { ok: false, error: error.message };
  await logAudit({ actorId: actor.id, actorName: actor.name, action: "deleted", entity: "marketer_log", label: id });
  revalidatePath("/marketer-log");
  return { ok: true };
}
