...
function Fields({ log, users, today }: { log?: MarketerLog; users: Opt[]; today: string }) {
  return (
    <>
      <div className="row">
...
function AddForm({ users, today }: { users: Opt[]; today: string }) {
  ...
  function submit(fd: FormData) {
    setErr(null);
    start(async () => {
      const r = await createMarketerLog(fd);
      if (!r.ok) {
...
function EditForm({ log, users, today, onDone }: { log: MarketerLog; users: Opt[]; today: string; onDone: () => void }) {
  const [err, setErr] = useState<string | null>(null);
  const [pending, start] = useTransition();
  const router = useRouter();
  ...
  function submit(fd: FormData) {
    setErr(null);
    start(async () => {
      const r = await updateMarketerLog(log.id, fd);
      if (!r.ok) {
        setErr(r.error ?? "저장 실패");
        return;
      }
...
function LogRow({ log, users, today }: { log: MarketerLog; users: Opt[]; today: string }) {
  ...
  function remove() {
    ...
    start(async () => {
      await deleteMarketerLog(log.id);
      router.refresh();
    });
  }
...
export default function MarketerLogClient({
  logs,
  users,
  today,
  dbReady,
  ...
  const tabs = ["전체", ...MARKETER_LOG_KINDS];
