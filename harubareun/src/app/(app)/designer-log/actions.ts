...
function isMissingTable(err: { code?: string; message?: string } | null): boolean {
  if (!err) return false;
  return err.code === "42P01" || /designer_logs/.test(err.message ?? "");
}
...
function normKind(v: FormDataEntryValue | null): string {
  ...
  return (DESIGNER_LOG_KINDS as readonly string[]).includes(s) ? s : "일일업무일지";
}

export async function createDesignerLog(formData: FormData): Promise<Result> {
  let user;
  try {
...
export async function updateDesignerLog(id: string, formData: FormData): Promise<Result> {
  try {
    await guardUser();
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : "권한 오류" };
  ...
  const { error } = await supabase.from("designer_logs").update(patch).eq("id", id);
  if (error) return { ok: false, error: error.message };
  revalidatePath("/designer-log");
  return { ok: true };
}

export async function deleteDesignerLog(id: string): Promise<Result> {
  let actor;
  try {
  ...
  const supabase = createSupabaseServerClient();
  const { error } = await supabase.from("designer_logs").delete().eq("id", id);
  if (error) return { ok: false, error: error.message };
  await logAudit({ actorId: actor.id, actorName: actor.name, action: "deleted", entity: "designer_log", label: id });
  revalidatePath("/designer-log");
  return { ok: true };
}
