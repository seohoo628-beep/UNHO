...
// 커머스 운영 프레임 — 강의 프레임을 하루바른(건기식·일반식품)·나아(화장품) D2C에 맞춰 재구성한 내부 도구.
...
const inputStyle: React.CSSProperties = { width: "100%", padding: "9px 11px", border: "1px solid var(--line-2)", borderRadius: "var(--radius, 8px)", background: "var(--surface)", color: "var(--ink)", fontSize: 14 };
  ...
  return (
    <div>
      ...
      <div className="card" style={card}>
        ...
        <div className="muted" style={{ fontSize: 11.5, marginTop: 8 }}>※ 최상급(최고·최초·최대·최저)은 표시광고법상 실증자료가 있을 때만 사용.</div>
      </div>
    </div>
  );
}
...
// ── 3. 원가·손익 ─────────────────────────────────────────────
function classifyRate(rate: number | null): { label: string; color: string; note: string } | null {
  if (rate == null) return null;
  if (rate <= 35) return { label: "메인 제품", color: "#10b981", note: "수익 창출·광고 여력 확보 (원가율 30% 내외 · 판매가 3만원대)" };
  if (rate >= 45 && rate <= 58) return { label: "서브(기획) 제품", color: "#f59e0b", note: "객단가 인상용 (원가율 50% 내외 · 5만원대 구성)" };
  if (rate >= 62) return { label: "미끼 제품", color: "#8b5cf6", note: "신규 유입·첫 구매 전환 (원가율 70% 내외 · 1만원대)" };
      ...
      <div className="card" style={{ padding: 0, overflowX: "auto", marginTop: 4 }}>
        <table className="tbl">
          ...
          <tbody>
            ...
            <tr><td>메인 제품</td><td>30% 내외</td><td>3만원대</td><td>수익 창출·광고 여력 확보</td></tr>
            <tr><td>서브(기획) 제품</td><td>50% 내외</td><td>5만원대 구성</td><td>객단가 인상</td></tr>
          </tbody>
        </table>
      </div>
      <div className="muted" style={{ fontSize: 11.5, marginTop: 8 }}>원가율과 판매가가 맞지 않으면 광고룸이 사라진다. 공장 견적 시점에 손익 구조를 먼저 대입하고, 구조가 안 나오면 사양을 바꾼다.</div>
    </div>
  );
}
...
const CHECKLISTS: { title: string; note?: string; items: string[] }[] = [
  {
    title: "상세페이지 체크리스트",
    note: "나아(화장품)·하루바른(건기식·일반식품)은 효능·기능성 표현 규제 대상. 비포·애프터는 브랜드별 가부가 다르므로 확정 전 런칭준비 상세페이지 수정안(A/B/C)으로 검수.",
    items: [
  ...
  },
  {
    ...
    note: '셋 중 하나라도 빠지면 이벤트가 아니라 그냥 할인. 예) 신학기 기념 → 3일 한정 → 선착순 200개. (이벤트 관리 폴더에서 3요소로 등록)',
    items: [
      "4~6주치 이벤트 캘린더 미리 편성",
    ...
    <div>
      ...
      <div className="card" style={{ ...card, borderLeft: "4px solid var(--owner,#b91c1c)" }}>
        ...
        <ul style={{ margin: "8px 0 0", paddingLeft: 18, display: "flex", flexDirection: "column", gap: 5 }}>
          ...
          <li style={{ fontSize: 13.5 }}>최상급 표현(최고·최초·최대·최저)은 실증자료 있는 항목에만. &lsquo;한정&rsquo;·명분·기간·수량 구조는 자유</li>
        </ul>
        <div className="muted" style={{ fontSize: 11.5, marginTop: 6 }}>하루바른(건기식·일반식품)·나아(화장품)은 적발 시 제재가 제조·판매 단계까지 번진다. 리뷰는 합법 수단으로만.</div>
      </div>
    </div>
  );
}
...
// ── 6. 우선 실행 항목 ────────────────────────────────────────
const PRIORITY = [
  { t: "하루바른 4종 3개월·6개월 정기구독 구성 설계", why: "1~2개월 재구매 주기로 정기구독 적합도 최상" },
  { t: "나아 세트·리필 라인 + 자외선 시즌 캠페인 연동", why: "화장품 재구매를 계절 수요와 결합해 광고 효율 개선" },
  { t: "브랜드 2축 상표권 등록 상태 전수 점검", why: "네이버 브랜드 심사·쿠팡 화이트리스트 진입 전제" },
    ...
    <div>
      <div className="page-head">
        <div>
          ...
          <p className="muted" style={{ margin: "2px 0 0", fontSize: 13 }}>강의 프레임을 하루바른(건기식·일반식품)·나아(화장품) D2C에 맞춰 재구성 · 내부용</p>
        </div>
      </div>
