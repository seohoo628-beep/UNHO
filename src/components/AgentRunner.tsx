...
type AgentType = "marketer" | "md" | "designer";
...
export default function AgentRunner({ brands }: { brands: Brand[] }) {
  const [agent, setAgent] = useState<AgentType>("marketer");
  ...
  async function run(all: boolean) {
    ...
    try {
      const res = await fetch("/api/agents/run", {
        ...
        body: JSON.stringify(
          all ? { agentType: agent, all: true } : { agentType: agent, brandId, promo }
        ),
      });
      ...
      if (!res.ok) {
        ...
        setMsg(`${AGENT_LABEL[agent]} 전체 실행 완료 — 승인 큐 ${q}건, 검수 미통과 ${b}건.`);
      } else {
      ...
      <div className="row" style={{ alignItems: "end" }}>
        <label className="field" style={{ marginBottom: 0 }}>
          ...
          <select value={agent} onChange={(e) => setAgent(e.target.value as AgentType)}>
            <option value="marketer">마케터 (릴스·피드)</option>
            <option value="md">MD (셀러 매칭·제안)</option>
            <option value="designer">디자이너 (지시서)</option>
          </select>
        </label>
        <label className="field" style={{ marginBottom: 0 }}>
          <span>브랜드</span>
