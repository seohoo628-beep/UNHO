...
export async function moveTodoChecklistItem(fromId: string, toId: string, itemId: string): Promise<Result> {
  ...
  if (error) return { ok: false, error: error.message };
  const norm = (v: any) => (Array.isArray(v) ? v : []).map((c: any) => ({ id: String(c?.id ?? ckId()), text: String(c?.text ?? "").trim(), done: !!c?.done, pinned: !!c?.pinned, brand: c?.brand ? String(c.brand) : null, due: /^\d{4}-\d{2}-\d{2}$/.test(String(c?.due ?? "")) ? String(c.due) : null })).filter((c: any) => c.text);
