...
function row(fd: FormData) {
  return {
    kind: String(fd.get("kind") ?? "").trim() || "시술",
...
export async function createLog(fd: FormData): Promise<Result> {
  ...
  let { error } = await supabase.from("antiaging_logs").insert(base);
  if (error && (error.code === "42703" || /kind/.test(error.message ?? ""))) {
    const { kind, ...rest } = base as any; void kind;
    ({ error } = await supabase.from("antiaging_logs").insert(rest));
  }
...
export async function updateLog(id: string, fd: FormData): Promise<Result> {
  ...
  const full = { ...row(fd), updated_at: new Date().toISOString() };
  let { error } = await supabase.from("antiaging_logs").update(full).eq("id", id);
  if (error && (error.code === "42703" || /kind/.test(error.message ?? ""))) {
    const { kind, ...rest } = full as any; void kind;
    ({ error } = await supabase.from("antiaging_logs").update(rest).eq("id", id));
  }
